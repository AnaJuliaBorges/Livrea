...
Required env vars (`.env`, not committed): `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_DEFAULT_KEY`, `VITE_GOOGLE_BOOKS_API_KEY`. Optional: `VITE_SENTRY_DSN` — without it `initSentry()` (`src/lib/sentry.ts`) no-ops, so dev/CI/tests never send events. On Vercel the Sentry integration provides `SENTRY_DSN`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` automatically; these are **build-time** vars (never `VITE_*`, which would ship the token in the bundle), and `vite.config.ts` maps `SENTRY_DSN` — and only it — to `VITE_SENTRY_DSN` so the client can read it. With no token the build skips both the upload and emitting `.map` files. The ISBNDB key is NOT a frontend var — it lives as the `ISBNDB_API_KEY` secret of the `isbndb` Edge Function (`supabase/functions/isbndb`), which proxies all ISBNDB calls so the paid key never ships in the bundle.
...
Supabase is the backend: Postgres tables/RPCs plus Auth and Storage (public buckets `avatars`, `club-covers`). The DB schema is **English snake_case** (`clubs.name`, `visibility`, `city_id`; RPC params prefixed `p_*`). Most reads/writes go through **SECURITY DEFINER RPCs** called with `supabase.rpc("name", { p_param })`, with authorization checks (membership/admin) inside the SQL function, not in the client. Service modules group RPCs by aggregate (e.g. `clubs/services/clubs.ts`, `clubMembers.ts`, `joinRequests.ts`, `clubReadings.ts`, `meetings.ts`) — add a new RPC call to the aggregate it belongs to, not a new one-function file. Each aggregate has one matching `*.test.ts` (`clubs.test.ts`, etc.) with a `describe` block per function; they mock `@/lib/supabase`, not the service module, and share one `rpcResult(data = null, error = null)` helper (success = `rpcResult()` / `rpcResult(payload)`, failure = `rpcResult(null, new Error(...))`). SQL scripts live temporarily in `supabase/sql/` (they're run manually in the SQL Editor and deleted after applying — the folder holds only pending scripts, it's not history). Server-side code lives in `supabase/functions/` (Deno Edge Functions): `send-push` (web-push notifications, validates everything against the DB via service role) and `isbndb` (authenticated proxy that holds the paid ISBNDB key as a secret).
